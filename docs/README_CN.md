...
过程：研究委员会先判断入场条件过度确认，然后依次测试 `entry.logic: and -> or`、放宽止损、延长持仓天数。每次修改都必须复测后才接受。完整报告见：[showcase_rsi_reversion_real_snapshot.md](reports/showcase_rsi_reversion_real_snapshot.md)。
...
Round 4 │ rsi_reversion_v4 │ 信号: 57 │ 评分: 37.7% 🏆
```
